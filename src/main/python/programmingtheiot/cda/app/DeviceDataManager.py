...
class DeviceDataManager(IDataMessageListener):
	...
	def __init__(self):
		...
		self.enableHandleTempChangeOnDevice = self.configUtil.getBoolean(section = ConfigConst.CONSTRAINED_DEVICE, key = ConfigConst.HANDLE_TEMP_CHANGE_ON_DEVICE_KEY)
		self.triggerHvacTempFloor = self.configUtil.getFloat(section = ConfigConst.CONSTRAINED_DEVICE, key = ConfigConst.TRIGGER_HVAC_TEMP_FLOOR_KEY)
		self.triggerHvacTempCeiling = self.configUtil.getFloat(section = ConfigConst.CONSTRAINED_DEVICE, key = ConfigConst.TRIGGER_HVAC_TEMP_CEILING_KEY)

		enableMqttClinet = self.configUtil.getBoolean(section = ConfigConst.CONSTRAINED_DEVICE, key = ConfigConst.ENABLE_MQTT_CLIENT_KEY)
	...
	def handleActuatorCommandMessage(self, data: ActuatorData) -> bool:
		...
		Memo: GDA -> (this function) -> actuator
	...
	def handleIncomingMessage(self, resourceEnum: ResourceNameEnum, msg: str) -> bool:
		...
		@return boolean
		"""
		logging.debug("Processing Imcoming message.")
		self._handleIncomingDataAnalysis(None)
		return True
	...
	def handleSensorMessage(self, data: SensorData) -> bool:
		...
		Memo: Sensor -> (this function) -> (_handleSensorDataAnalysis) -> actuator
	...
	def handleSystemPerformanceMessage(self, data: SystemPerformanceData) -> bool:
		...
		Memo: SystemPerformance -> (this function) -> GDA
		
		@param data The incoming SystemPerformanceData message.
		@return boolean
		"""
		if data:
			logging.debug("Processing system performance data.")
			self._handleUpstreamTransmission(ResourceNameEnum.CDA_SYSTEM_PERF_MSG_RESOURCE, None)
			return True
		else:
			logging.warning("Invalid system performance data. Ignoring")
			return False
